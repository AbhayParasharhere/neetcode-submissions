class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        while l<=r:
            m = (l+r) // 2
            # a end is count of elements from A from 0 elem to all or n
            # b end is count 
            Acount = m
            Bcount = half - m

            Alt = A[Acount-1] if Acount-1 >= 0 else float('-inf')
            Art = A[Acount] if Acount < len(A) else float('inf')
            Blt = B[Bcount-1] if Bcount-1 >= 0 else float('-inf')
            Brt = B[Bcount] if Bcount< len(B) else float('inf')
